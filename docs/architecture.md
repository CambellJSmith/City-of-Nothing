...
Lighting remains a screen-space post-process. After the world render, `draw_light()` applies the appropriate indoor, sewer, or time-of-day darkness to a reusable transparent canvas, gathers active visible light definitions, and cuts radial gradients out of that isolated overlay. The completed overlay is then drawn over the opaque world canvas, so subtractive blending can never erase or blacken world pixels. A missing `cone` produces circular coverage. Directional definitions clip the same falloff to the item's saved quarter-turn rotation. Range, cone width, strength, tint, power, and flicker remain catalog data rather than separate rendering branches.
